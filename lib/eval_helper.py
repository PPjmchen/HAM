...
def get_eval(data_dict, config, reference, use_lang_classifier=False, use_oracle=False, use_cat_rand=False, use_best=False, post_processing=None, proposal_method='scanrefer'):
    """ Loss functions
    ...
    if proposal_method == 'scanrefer' or proposal_method == 'detr3d':
        objectness_preds_batch = torch.argmax(data_dict['objectness_scores'], 2).long()
        objectness_labels_batch = data_dict['objectness_label'].long()
    elif proposal_method == 'groupfree':
        # objectness_preds_batch = torch.argmax(data_dict['proposal_objectness_scores'], 2).long()
        objectness_preds_batch = torch.sigmoid(data_dict['last_objectness_scores']).round().squeeze()
    ...
    if proposal_method == 'scanrefer' or proposal_method == 'detr3d':
        ...
        # obj_pred_val = torch.argmax(data_dict['proposal_objectness_scores'], 2) # B,K
        obj_pred_val = torch.sigmoid(data_dict['last_objectness_scores']).round().squeeze()
        obj_acc = torch.sum((obj_pred_val==data_dict['last_objectness_label'].long()).float()*\
            torch.ones_like(data_dict['last_objectness_mask']))/(torch.sum(torch.ones_like(data_dict['last_objectness_mask']))+1e-6)
        data_dict['obj_acc'] = obj_acc
        # detection semantic classification
